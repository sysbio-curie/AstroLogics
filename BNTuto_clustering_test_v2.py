...
for x in tqdm(range(0, 100)):
    rand_index_score = []
    # Euclidean k-means
    kmeans = KMeans(n_clusters=75, random_state=x)
    kmeans.fit(distance_matrix)
    clusters = kmeans.labels_
    model_name = list(distance_matrix.index)
    ...
    for i in range(0, len(attractor_combo_list)-1):
        ...
        model_name = list(distance_matrix_sub.index)
        ...
        cluster_df = cluster_df.loc[cluster_df.index.isin(model_name)]
...
# Save the results
total_index_score.to_csv("/home/spankaew/Git/astrologics/tmp/rand_index_endpointclustering_v2.csv", index = False)